...
- Package: `dev.<username>.grayout` (replace with actual)
...
- Use Material 3 theming. All colors/typography via theme, no hardcoded values.
...
- Read DESIGN.md for design tokens, component patterns, and general rules.
- Read the relevant file in docs/screens/ for screen-specific layouts.
  (For MVP, screen specs are in DESIGN.md directly. They'll be split out later.)
- All UI must use theme tokens from DESIGN.md. No hardcoded colors/sizes.
- When building a new screen, check if a screen spec exists before starting.
...
## File Organization
```
app/src/main/java/dev/<username>/grayout/
