...
write left the display gray. Failed display writes retain preference flags for retry
...
Screen interactivity is checked inside the transition lock. With the screen off,
the new schedule state is applied immediately and the suspended exclusion is cleared
only after success. A later screen-on entry captures that new state for its eventual
exit. This covers a schedule starting after the user locked the phone from an
excluded app that had already been in color.
...
- The updated app passes all 25 Android instrumentation tests on that emulator,
  including eight schedule/exclusion integration cases using real broadcasts, Room,
  secure settings, and actual screen sleep/wake events.
- All 204 JVM unit tests pass. New cases cover boundary ordering, repeated resync,
  ...
  entry/start and exit/end operations, plus screen-off boundaries and failed writes
  followed by wake/re-entry or restart reconciliation.
...
the successful 25-test result is from the emulator only.
...
record. A subsequent adversarial pass found the screen-off deferred-start case;
the interactivity check and sleep/wake regressions address it. Merge requires all
three votes to be yes.
